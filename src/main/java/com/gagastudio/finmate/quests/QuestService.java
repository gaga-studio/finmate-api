...
public class QuestService {
	private static final List<QuestSeed> SEEDS = List.of(
		new QuestSeed("DAILY_SPENDING_CHECK", "Daily spending check", "BEHAVIOR", 10, 5),
		new QuestSeed("WEEKLY_SAVING_PLAN", "Weekly saving plan", "BEHAVIOR", 20, 8),
		new QuestSeed("PUBLIC_TRANSIT", "Choose public transit", "BEHAVIOR", 15, 6),
		new QuestSeed("RISK_PROFILE", "Complete risk-profile diagnosis", "BEHAVIOR", 20, 5),
		new QuestSeed("ETF_OX_QUIZ", "ETF O/X knowledge quiz", "BEHAVIOR", 15, 5),
		new QuestSeed("KNOWLEDGE_STREAK_7", "Seven-day knowledge streak", "SYNTHETIC_MYDATA", 30, 10));
	...
	QuestDtos.QuestPage list(UUID userId) {
		...
		if (userQuests.isEmpty()) {
			...
			for (int index = 0; index < SEEDS.size(); index++) {
				...
				quests.save(new Quest(userId, seed.code(), index, seed.title(), seed.verificationKind(),
					seed.xpReward(), seed.pointReward(), now));
			}
	...
	CompletionResult complete(UUID userId, UUID questId, String idempotencyKey) {
		...
		if (replay != null) {
			...
			return completionResult(userId, questId, replay.getXpAwarded(), replay.getPointsAwarded(),
				quest.getStatus().equals("DATA_PENDING"));
		}
		...
		if (quest.getVerificationKind().equals("SYNTHETIC_MYDATA")) {
			...
			completions.save(new QuestCompletion(userId, questId, idempotencyKey, 0, 0, now));
			return new CompletionResult(new QuestDtos.QuestCompletionView(view(quest), 0, 0, false), true);
		}
		Instant now = Instant.now();
		quest.markCompleted(now);
		int pointsAwarded = points.awardQuestPoints(userId, questId, quest.getPointReward(), now);
		completions.save(new QuestCompletion(userId, questId, idempotencyKey, quest.getXpReward(), pointsAwarded, now));
		records.appendQuestCompletion(userId, now, quest.getTitle(), quest.getXpReward());
		return new CompletionResult(new QuestDtos.QuestCompletionView(view(quest), quest.getXpReward(), pointsAwarded, false), false);
	}
	...
	private record QuestSeed(String code, String title, String verificationKind, int xpReward, int pointReward) {
	}
}
