...
CREATE TABLE finmate_synthetic_financial_snapshot (
    ...
    xp INTEGER NOT NULL,
    last_synced_at TIMESTAMPTZ NOT NULL,
    CONSTRAINT finmate_snapshot_goal_owner_fk
        FOREIGN KEY (user_id, goal_id) REFERENCES finmate_user_goal(user_id, id),
...
CREATE INDEX finmate_snapshot_month_idx
    ON finmate_synthetic_financial_snapshot (user_id, goal_id, snapshot_month, last_synced_at DESC);
...
CREATE TABLE finmate_raid_projection (
    ...
    CONSTRAINT finmate_raid_goal_owner_fk
        FOREIGN KEY (user_id, goal_id) REFERENCES finmate_user_goal(user_id, id),
...
CREATE TABLE finmate_raid_projection_audit (
    ...
    CONSTRAINT finmate_raid_audit_owner_fk
        FOREIGN KEY (user_id, goal_id, raid_id) REFERENCES finmate_raid_projection(user_id, goal_id, id),
    CHECK (current_progress_bps BETWEEN 0 AND 10000),
    CHECK (highest_progress_bps BETWEEN current_progress_bps AND 10000),
    CHECK (stage BETWEEN 1 AND 3),
    CHECK (boss_hp_bps BETWEEN 0 AND 10000)
